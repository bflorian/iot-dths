 ...
 */
metadata {
	definition (name: "Aeon Home Energy Meter (local)", namespace: "smartthings", author: "SmartThings", 
        ...
        capability "angelpanel53748.powerMeter"
	...
	tiles(scale: 2) {
		...
		standardTile("reset", "device.energy", inactiveLabel: false, decoration: "flat",width: 2, height: 2) {
			state "default", label:'reset kWh', action:"push"
		}
...
def updated() {
	log.debug "updated()..."
    unschedule()
    schedule("0 0 * * * ?", hourHandler)
	response(refresh())
}

def hourHandler() {
	log.trace("hourHandler()")
	sendHubCommand([response(resetEnergyMeter())])
}
...
private meterReport(scale, value) {
	if (scale == 0) {
    	...
    	sendEvent(name: "angelpanel53748.powerMeter.power", value: Math.round(value), unit: "W")
		return [name: "power", value: Math.round(value), unit: "W"]
	}
}
