...
metadata {
	...
	definition(name: "SmartPower Outlet (local)", namespace: "smartthings", author: "SmartThings", mnmn: "SmartThings", vid: "generic-switch-power", ocfDeviceType: "oic.d.smartplug", runLocally: true, minHubCoreVersion: '000.017.0012', executeCommandsLocally: false) {
		capability "Actuator"
		capability "Switch"
...
def parse(String description) {
	...
	if (event) {
		if (event.name == "power") {
			...
			def lastPowerValue = device.currentState("power")?.floatValue ?: 0
			if (Math.abs(lastPowerValue - value) / lastPowerValue > 0.10 || lastPowerValue != 0 && value == 0) {
