...
metadata {
    definition (name: "SmartWeather Station Tile", namespace: "smartthings", author: "SmartThings") {
        ...
        capability "bobflorian.precipitationProbability"
        ...
        //attribute "iconToday", "string"
        //attribute "iconTonight", "string"
        //attribute "iconTomorrow", "string"
    ...
    tiles(scale: 2) {
        ...
        valueTile("pop", "device.pop", decoration: "flat", height: 1, width: 2) {
        ...
        details(["temperature", "feelsLike", "weatherIcon", "humidity", "wind",
                 "weather", "city", "pop", "ultravioletIndex", "light",
                 "rise", "set",
                 "refresh",
...
def initialize() {
    if (!childDevices) {
        addChildDevice(
            ...
            "${device.deviceNetworkId}.low",
            null,
            [completedSetup: true, label: "${device.label} Low Temperature", componentName: "lowTemperature", componentLabel: "Low Temperature"])
        addChildDevice(
            "SmartWeather Station Temperature",
            "${device.deviceNetworkId}.high",
            null,
            [completedSetup: true, label: "${device.label} High Temperature", componentName: "highTemperature", componentLabel: "High Temperature"])
    }
    poll()
}

def getLowTemperatureComponent() {
    return childDevices.find {it.componentName == 'lowTemperature'}
}

def getHighTemperatureComponent() {
    return childDevices.find {it.componentName == 'highTemperature'}
}
...
def pollUsingZipCode(String zipCode) {
    ...
    if (obs) {
        ...
        if (f) {
            ...
            send(name: "pop", value: value as String, unit: "%")
            ...
            send(name: "iconToday", value: f.daypart[0].iconCode[0])
            send(name: "iconTonight", value: f.daypart[0].iconCode[1])
            send(name: "iconTomorrow", value: f.daypart[0].iconCode[2])
            lowTemperatureComponent.sendEvent(name: "temperature", value: f.temperatureMin[0] ?: f.temperatureMin[1], unit: tempUnits)
            highTemperatureComponent.sendEvent(name: "temperature", value: f.temperatureMax[0] ?: f.temperatureMax[1], unit: tempUnits)
        }
        else {
