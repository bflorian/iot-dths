...
metadata {
    definition (name: "Inovelli Dimmer Red Series LZW31-SN (local MCD)", namespace: "bflorian", author: "Bob Florian", 
    	mnmn: "SmartThingsCommunity", vid: "1948049f-973b-3731-91d0-a4049f2187ee") {
...
def initialize() {
    ...
    //if (!device.currentValue("supportedButtonValues")) {
        def value = ['pushed', 'held', 'up', 'up_2x', 'up_3x', 'up_4x', 'up_5x', 'down', 'down_2x', 'down_3x', 'down_4x', 'down_5x'].encodeAsJson()
...
def sendButtonEvent(gesture) {
    log.trace gesture
    sendEvent(name: "button", value: gesture, data: [buttonNumber: 1], isStateChange: true)
}
...
def zwaveEvent(physicalgraph.zwave.commands.centralscenev1.CentralSceneNotification cmd) {
    ...
    if (cmd.sceneNumber == 1) {
        if (cmd.keyAttributes == 0) {
            ...
            log.debug("Unhandled scene notification, keyAttributes=${cmd.keyAttributes}, sceneNumber=${cmd.sceneNumber}")
        } else if (cmd.keyAttributes == 2) {
            log.debug("Unhandled scene notification, keyAttributes=${cmd.keyAttributes}, sceneNumber=${cmd.sceneNumber}")
        } else {
            def number = cmd.keyAttributes - 1
            sendButtonEvent("down_${number}x")
        }
        ...
        if (cmd.keyAttributes == 0) {
            ...
            def number = cmd.keyAttributes - 1
            sendButtonEvent("up_${number}x")
        }
    } else if (cmd.sceneNumber == 3) {
        if (cmd.keyAttributes == 0) {
        ...
        log.warn("Unhandled scene notification, keyAttributes=${cmd.keyAttributes}, sceneNumber=${cmd.sceneNumber}")
    }
}
